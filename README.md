At branch of lineage-22.0, following typical local manifest can use.  
Especially, most important point is following to use <span style="color: red; ">"remove-project path="</span><br> 
When using <span style="color: red; ">"remove-project name"</span>, there is a concern that it may break the state  
in which different branches are deployed to directories with different sources in the same repository.
...
<manifest>
  ...
  <!--  remove-project path -->
  <remove-project path="hardware/qcom-caf/sm8250/display" />    
  ...
  <project path="device/asus/zenfone7"                           name="omnirom_device_asus_zenfone7"                             remote="BrightTwikling"  revision="lineage-15.0"      />
